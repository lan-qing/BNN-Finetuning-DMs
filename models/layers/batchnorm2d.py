...
class RandBatchNorm2d(nn.Module):
    def __init__(self, num_features, init_s=-1e10, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True):
    ...
    def forward(self, input, sample=True, fix=False):
        ...
        if self.affine:
            ...
            if fix:
                ...
                eps_weight = torch.Tensor(*self.eps_weight.size()).normal_().cuda().type(sig_weight.dtype)
            ...
            if fix:
                ...
                eps_bias = torch.Tensor(*self.eps_bias.size()).normal_().cuda().type(sig_bias.dtype)
