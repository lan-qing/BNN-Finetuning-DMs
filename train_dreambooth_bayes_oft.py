...
from oft_utils.attention_processor import OFTRandAttnProcessor, OFTAttnProcessor
...
def main(args):
    ...
    for name in unet.attn_processors.keys():
        ...
        if (not args.skip_up_block and name.startswith("up_blocks")) or (
            not args.skip_mid_block and name.startswith("mid_blocks")) or (
            not args.skip_down_block and name.startswith("down_blocks")):
            oft_attn_procs[name] = OFTRandAttnProcessor(hidden_size=hidden_size,
                                                        cross_attention_dim=cross_attention_dim,
                                                        eps=args.eps, r=args.r, is_coft=args.coft,
                                                        init_s=args.init_sigma)
        else:
            oft_attn_procs[name] = OFTAttnProcessor(hidden_size=hidden_size,
                                                    cross_attention_dim=cross_attention_dim,
                                                    eps=args.eps, r=args.r, is_coft=args.coft)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.lambda1 > 0.0:
                    ...
                    loss2 = bayesian.cal_KL_modules(oft_layers, prior_mu, prior_sigma=args.prior_sigma)
