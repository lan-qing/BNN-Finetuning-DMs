...
class OFTRandLinearLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=False, block_share=False, eps=6e-5, r=4, is_coft=False,
                 init_s=0.01):
        super(OFTRandLinearLayer, self).__init__()
        ...
        if self.block_share:
            # Initialized as an identity matrix
            self.R_shape = [in_features // self.r, in_features // self.r]
            self.mu_R = nn.Parameter(torch.zeros(self.R_shape[0], self.R_shape[0]), requires_grad=True)
            self.sigma_R = nn.Parameter(init_s * torch.ones(self.R_shape[0], self.R_shape[0]), requires_grad=True)
            ...
            R = torch.zeros(self.R_shape[1], self.R_shape[1])
            R = torch.stack([R] * self.r)
            self.mu_R = nn.Parameter(R, requires_grad=True)
            self.sigma_R = nn.Parameter(torch.ones_like(R) * init_s, requires_grad=True)
            self.eps = eps * self.R_shape[1] * self.R_shape[1]

    def forward(self, attn, x, sample=True):
        orig_dtype = x.dtype
        dtype = self.mu_R.dtype

        if sample:
            R = self.mu_R + self.sigma_R * torch.randn_like(self.sigma_R)
        else:
            R = self.mu_R
...
class OFTRandAttnProcessor(nn.Module):
    def __init__(self, hidden_size, cross_attention_dim=None, eps=2e-5, r=4, is_coft=False, init_s=0.01):
        super().__init__()
        ...
        self.to_q_oft = OFTRandLinearLayer(hidden_size, hidden_size, eps=eps, r=r, is_coft=is_coft, init_s=init_s)
        self.to_k_oft = OFTRandLinearLayer(cross_attention_dim or hidden_size, hidden_size, eps=eps, r=r,
                                           is_coft=is_coft, init_s=init_s)
        self.to_v_oft = OFTRandLinearLayer(cross_attention_dim or hidden_size, hidden_size, eps=eps, r=r,
                                           is_coft=is_coft, init_s=init_s)
        self.to_out_oft = OFTRandLinearLayer(hidden_size, hidden_size, eps=eps, r=r, is_coft=is_coft, init_s=init_s)
